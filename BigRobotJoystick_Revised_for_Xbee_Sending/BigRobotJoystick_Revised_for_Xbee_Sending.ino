...
const unsigned long TIME_BETWEEN_GET_DATA = 100;    // millis.  perhaps reduce to 50?
...
void setup()
{
   ...
   buttonState = digitalRead(BUTTON);   // read the initial button state
...
{ 
   ...
   if (valButton != buttonState)     // the button state has changed!
   { 
     if (valButton == LOW)           // check if the button is pressed
     {    
        buttonPresses++;             // increment the buttonPresses variable
     }
     buttonState = valButton;        // save the new button state in our variable
   }
